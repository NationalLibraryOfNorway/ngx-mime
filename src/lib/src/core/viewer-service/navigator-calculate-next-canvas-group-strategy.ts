...
export class NavigatorCalculateNextCanvasGroupStrategy implements CalculateNextCanvasGroupStrategy {
  calculateNextCanvasGroup(criteria: NextCanvasGroupCriteria): number {
    ...
    nextCanvasGroup = direction === Direction.NEXT ? nextCanvasGroup : nextCanvasGroup * -1;
